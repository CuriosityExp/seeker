...
class JobController {
  ...
  static async generateRoles(req, res, next) {
    try {
      ...
      if (!dataProfile) throw { name: "CustomError", status: 404, message: "Profile not found" };

      const dataExperience = await WorkExperience.findAll({
        where: { ProfileId: dataProfile.id },
      });

      if (!dataExperience) throw { name: "CustomError", status: 404, message: "Work Experience not found" };

      const dataEducation = await Education.findAll({
        where: { ProfileId: dataProfile.id },
      });

      if (!dataEducation) throw {
        name: "CustomError",
        status: 404,
